...
def get_current_status():
    orderbook = pyupbit.get_orderbook(ticker="KRW-CVC")
    ...
    for b in balances:
        if b['currency'] == "CVC":
            CVC_balance = b['balance']
            CVC_avg_buy_price = b['avg_buy_price']
        if b['currency'] == "KRW":
            krw_balance = b['balance']

    current_status = {'current_time': current_time, 'orderbook': orderbook, 'CVC_balance': CVC_balance, 'krw_balance': krw_balance, 'CVC_avg_buy_price': CVC_avg_buy_price}
...
def analyze_data_with_gpt4(data_json):
    instructions_path = "instructionsCVC.md"
    try:
...
def execute_buy():
    print("Attempting to buy CVC...")
    try:
        krw = upbit.get_balance("KRW")
        if krw > 5000:
            result = upbit.buy_market_order("KRW-CVC", krw*0.9995)
            print("Buy order successful:", result)
    except Exception as e:
        print(f"Failed to execute buy order: {e}")

def execute_sell():
    print("Attempting to sell CVC...")
    try:
        CVC = upbit.get_balance("CVC")
        current_price = pyupbit.get_orderbook(ticker="KRW-CVC")['orderbook_units'][0]["ask_price"]
        if current_price*CVC > 5000:
            result = upbit.sell_market_order("KRW-CVC", CVC)

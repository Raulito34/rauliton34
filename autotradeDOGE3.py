...
def get_current_status():
    orderbook = pyupbit.get_orderbook(ticker="KRW-DOGE")
    current_time = orderbook['timestamp']
    DOGE_balance = 0
    krw_balance = 0
    DOGE_avg_buy_price = 0
    balances = upbit.get_balances()
    for b in balances:
        if b['currency'] == "DOGE":
            DOGE_balance = b['balance']
            DOGE_avg_buy_price = b['avg_buy_price']
        if b['currency'] == "KRW":
            krw_balance = b['balance']

    current_status = {'current_time': current_time, 'orderbook': orderbook, 'DOGE_balance': DOGE_balance, 'krw_balance': krw_balance, 'DOGE_avg_buy_price': DOGE_avg_buy_price}
    return json.dumps(current_status)


def fetch_and_prepare_data():
    # Fetch data
    df_daily = pyupbit.get_ohlcv("KRW-DOGE", "day", count=30)
    df_hourly = pyupbit.get_ohlcv("KRW-DOGE", interval="minute60", count=24)
...
def analyze_data_with_gpt4(data_json):
    instructions_path = "instructionsDOGE.md"
    try:
...
def execute_buy():
    print("Attempting to buy DOGE...")
    try:
        krw = upbit.get_balance("KRW")
        if krw > 5000:
            result = upbit.buy_market_order("KRW-DOGE", krw*0.9995)
            print("Buy order successful:", result)
    except Exception as e:
        print(f"Failed to execute buy order: {e}")

def execute_sell():
    print("Attempting to sell DOGE...")
    try:
        DOGE = upbit.get_balance("DOGE")
        current_price = pyupbit.get_orderbook(ticker="KRW-DOGE")['orderbook_units'][0]["ask_price"]
        if current_price*DOGE > 5000:
            result = upbit.sell_market_order("KRW-DOGE", DOGE)

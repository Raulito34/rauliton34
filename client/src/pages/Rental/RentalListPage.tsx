...
const statusLabel: Record<RentalBooking['status'], string> = {
  reviewing: '심사중',
  confirmed: '대관완료',
};

const statusStyle: Record<RentalBooking['status'], string> = {
  reviewing: 'bg-orange-100 text-orange-700 border-orange-200',
  confirmed: 'bg-green-100 text-green-700 border-green-200',
};

export default function RentalListPage() {
  const [bookings, setBookings] = useState<RentalBooking[]>(getBookings);
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [filterStatus, setFilterStatus] = useState<'all' | RentalBooking['status']>('all');

  const refresh = () => setBookings(getBookings());

  const handleStatusChange = (id: string, status: RentalBooking['status']) => {
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
            ...
            <div className="bg-orange-50 rounded-lg p-5 text-center">
              <p className="text-2xl font-bold text-orange-600">{bookings.filter((b) => b.status === 'reviewing').length}</p>
            ...
            <div className="space-y-4">
              {sorted.map((b) => (
                ...
                >
                  ...
                  >
                    <span className={`text-xs font-medium px-2.5 py-1 rounded border ${statusStyle[b.status]}`}>
                    ...
                    <div className="border-t border-gray-100 px-6 py-5 bg-gray-50/50">
                      ...
                      <div className="flex flex-wrap items-center gap-3 mt-6 pt-4 border-t border-gray-200">
                        ...
                        {selectedBooking.status !== 'reviewing' && (
                          <button
                            onClick={() => handleStatusChange(selectedBooking.id, 'reviewing')}
